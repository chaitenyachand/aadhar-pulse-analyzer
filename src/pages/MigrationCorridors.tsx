...
export default function MigrationCorridors() {
  const { data: migrationData, isLoading } = useMigrationCorridors();
  ...
  const destinationStats = migrationData?.reduce((acc, c) => {
    if (!acc[c.to]) acc[c.to] = { state: c.to, inflow: 0, sources: 0 };
    acc[c.to].inflow += c.flow;
    acc[c.to].sources += 1;
    return acc;
  }, {} as Record<string, { state: string; inflow: number; sources: number }>) || {};
  ...
  const sourceStats = migrationData?.reduce((acc, c) => {
    if (!acc[c.from]) acc[c.from] = { state: c.from, outflow: 0, destinations: 0 };
    acc[c.from].outflow += c.flow;
    acc[c.from].destinations += 1;
    return acc;
  }, {} as Record<string, { state: string; outflow: number; destinations: number }>) || {};
    ...
    <DashboardLayout>
      ...
      <div className="p-8 space-y-8">
        ...
        <ChartCard
          ...
          subtitle="Sankey diagram showing inter-state migration patterns • Click for AI insights"
        ...
        {/* Decision Panel */}
        <DecisionPanel
          insight="Bihar and Uttar Pradesh show highest out-migration to Maharashtra and Delhi, indicating strong economic pull factors"
          policyAction="Establish mobile Aadhaar update camps at major destination points (Mumbai, Delhi) for migrant populations"
          operationalImpact="↑ 40% address update compliance, ↓ 25% pendency in source states"
          variant="default"
        />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <ChartCard
            ...
            subtitle="States with highest emigration • Click for AI insights"
            className="cursor-pointer hover:ring-2 hover:ring-primary/20 transition-all"
            onClick={() => setInsightModal({
              open: true,
              title: "Top Source States (Out-Migration)",
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={topSources} layout="vertical">
                  ...
                  <XAxis
                    ...
                    fontSize={12}
                    tickFormatter={(v) => formatIndianCompact(v)}
                  />
                  <YAxis
                    type="category"
                    dataKey="state"
          ...
          <ChartCard
            ...
            subtitle="States with highest immigration • Click for AI insights"
            className="cursor-pointer hover:ring-2 hover:ring-primary/20 transition-all"
            onClick={() => setInsightModal({
              open: true,
              title: "Top Destination States (In-Migration)",
        ...
        <ChartCard
          ...
          subtitle="Ranked by estimated migration volume with confidence scores"
        >
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {migrationData?.slice(0, 10).map((corridor, index) => (
                  <tr
