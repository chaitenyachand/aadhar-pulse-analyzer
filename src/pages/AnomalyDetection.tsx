...
import { AlertTriangle, Shield, Clock, MapPin, TrendingUp, TrendingDown, CheckCircle, XCircle, RefreshCw, Fingerprint, Eye, Activity } from "lucide-react";
...
export default function AnomalyDetection() {
  ...
  const biometricAgingPrediction = biometricByAge?.length ? [{
    ...
    ageGroup: "5-18",
    currentSuccess: biometricByAge[0]?.percentage || 88,
    projected5Year: 82,
    riskLevel: "medium"
  }, {
    ageGroup: "18-40",
    ...
    ageGroup: "60+",
    currentSuccess: biometricByAge[1]?.percentage || 82,
    projected5Year: 65,
    riskLevel: "high"
  }] : [];
  ...
  const welfareCorrelation = [{
    ...
    coverage: 92,
    successRate: 95,
    beneficiaries: Math.round(totalEnrollment * 0.15)
  }, {
    scheme: "NREGA",
    coverage: 88,
    successRate: 91,
    beneficiaries: Math.round(totalEnrollment * 0.10)
  }, {
    scheme: "PDS",
    coverage: 95,
    successRate: 97,
    beneficiaries: Math.round(totalEnrollment * 0.22)
  }, {
    scheme: "PM-JAY",
    coverage: 78,
    successRate: 88,
    beneficiaries: Math.round(totalEnrollment * 0.06)
  }, {
    scheme: "LPG Subsidy",
    coverage: 85,
    successRate: 93,
    beneficiaries: Math.round(totalEnrollment * 0.09)
  }];
  ...
  return <DashboardLayout>
      ...
      <div className="p-8 space-y-8">
        {/* Active Alerts */}
        <ChartCard title="Active Anomaly Alerts" subtitle="Real-time detection of unusual patterns requiring attention" actions={<Button variant="outline" size="sm" onClick={() => refetch()}>
        ...
        <ChartCard title="Trust Erosion Warning System" subtitle="6-month trend of trust indicators • Click for AI insights" className="cursor-pointer hover:ring-2 hover:ring-primary/20 transition-all" onClick={() => setInsightModal({
        open: true,
          ...
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={trustErosionData}>
                ...
                <Legend />
                <Area yAxisId="left" type="monotone" dataKey="trustScore" name="Trust Score" stroke={CHART_COLORS[2]} fill={CHART_COLORS[2]} fillOpacity={0.2} />
                <Line yAxisId="right" type="monotone" dataKey="biometricFailure" name="Biometric Failure %" stroke={CHART_COLORS[5]} strokeWidth={2} dot={{
                r: 4
              }} />
        ...
        {/* Decision Panel */}
        <DecisionPanel insight="Trust score declining 8% over 6 months, correlated with rising biometric failure rates in elderly population" policyAction="Deploy iris-based verification as primary modality for 60+ age group; implement proactive biometric re-enrollment" operationalImpact="↑ 15% authentication success in elderly, ↓ 30% trust erosion rate" variant="warning" />

        {/* Two Column Layout */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Biometric Aging Prediction */}
          <ChartCard title="Biometric Aging Prediction Model" subtitle="Projected 5-year biometric success rates by age group • Click for insights" className="cursor-pointer hover:ring-2 hover:ring-primary/20 transition-all" onClick={() => setInsightModal({
          open: true,
          ...
          data: biometricAgingPrediction
            ...
            <div className="mt-4 space-y-2">
              {biometricAgingPrediction.filter(d => d.riskLevel === "high").map(d => <div key={d.ageGroup} className="flex items-center gap-2 text-sm text-destructive bg-destructive/5 p-2 rounded">
                  <AlertTriangle className="w-4 h-4" />
          ...
          <ChartCard title="Welfare Scheme Impact Analysis" subtitle="Aadhaar authentication success rates across welfare programs • Click for insights" className="cursor-pointer hover:ring-2 hover:ring-primary/20 transition-all" onClick={() => setInsightModal({
          open: true,
          ...
          data: welfareCorrelation
            ...
            <div className="mt-4 grid grid-cols-2 gap-2 text-sm">
              {welfareCorrelation.slice(0, 4).map(w => <div key={w.scheme} className="flex items-center justify-between p-2 bg-muted/30 rounded">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <CompactDecisionPanel insight="0-5 age group fingerprint reliability at 12%" policyAction="Mandate face/iris for child enrollments" impact="↑ 68% success rate" />
          <CompactDecisionPanel insight="PM-JAY has lowest Aadhaar coverage at 78%" policyAction="Integrate assisted authentication at hospitals" impact="↑ 12% coverage, 2.3Cr beneficiaries" />
          <CompactDecisionPanel insight="60+ biometric projected to drop to 65%" policyAction="Deploy proactive re-enrollment camps" impact="Prevent 1.8Cr authentication failures" />
        </div>

        {/* Biometric by Age Distribution */}
        <ChartCard title="Current Biometric Modality Distribution by Age" subtitle="Success rates of fingerprint, iris, and face authentication across age groups">
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
